...
test_gateway_admin_health() {
    ...
    local url="${BASE_URL}:8081/actuator/health"
...
test_gateway_admin_info() {
    ...
    local url="${BASE_URL}:8081/actuator/info"
...
test_admin_ui_accessible() {
    ...
    local url="${BASE_URL}:3000/"

    if http_get "$url" "200"; then
        test_pass "Admin UI is accessible"
    else
        # Попробуем порт 80 (nginx в production)
        url="${BASE_URL}/"
        if http_get "$url" "200"; then
            test_pass "Admin UI is accessible (port 80)"
        else
            test_fail "Admin UI not accessible (tried :3000 and :80)"
        fi
    fi
}
...
test_admin_ui_assets() {
    ...
    if echo "$response" | grep -q "assets/"; then
        test_pass "Admin UI assets references found"
    else
        # Попробуем порт 80
        url="${BASE_URL}/"
        response=$(curl -sf --max-time "$TIMEOUT" "$url" 2>/dev/null || echo "")
...
test_swagger_ui() {
    ...
    local url="${BASE_URL}:8081/swagger-ui.html"

    if http_get "$url" "200"; then
        test_pass "Swagger UI is accessible"
    else
        # Попробуем redirect на swagger-ui/index.html
        url="${BASE_URL}:8081/swagger-ui/index.html"
        if http_get "$url" "200"; then
...
test_api_routes_endpoint() {
    ...
    local url="${BASE_URL}:8081/api/v1/routes"
