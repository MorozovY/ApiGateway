...
/**
 * Интеграционные тесты для HealthController (Story 8.1, 12.1, 13.8).
 *
 * Проверяет AC1, AC2:
 ...
 * gateway-core, keycloak, prometheus, grafana не запущены, поэтому будут DOWN.
 *
 * Примечание: nginx удалён в Story 13.8, reverse proxy — централизованный Traefik.
 */
...
class HealthControllerIntegrationTest {
    ...
    companion object {
        ...
        fun configureProperties(registry: DynamicPropertyRegistry) {
            ...
            registry.add("prometheus.url") { "http://localhost:58081" }  // порт недоступен
    ...
    inner class AC1_HealthServicesEndpoint {
        ...
        fun `GET health services возвращает 200 и все сервисы`() {
            // Backend возвращает 7 сервисов: 4 из AC (gateway-core, gateway-admin, postgresql, redis)
            // + 3 дополнительных (keycloak, prometheus, grafana)
            // Примечание: nginx удалён в Story 13.8
            webTestClient.get()
